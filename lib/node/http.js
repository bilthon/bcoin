...
class HTTP extends Server {
  ...
  /**
  * Function introduced exclusively for the SelfTrust system. The purpose
  * is to discard any extra information not currently being used by
  * our online component (web interface).
  */
  filterTxBulkResponse(tx) {
    ...
    if (tx.inputs.length > 0) {
      tx.inputs.forEach(input => {
        ...
        if (input.coin) {
          delete input.coin.script;
          delete input.coin.coinbase;
        }
      });
    }
    return tx;
  ...
  initRouter() {
    ...
    this.post('/tx/address', async (req, res) => {
      // The maximum numer of tx per address to include
      const MAX_TX_COUNT = 5000;
      const result = [];
      const valid = Validator.fromRequest(req);
      const addresses = valid.get('addresses');
      const limit = this.options.maxTxs;
      const reverse = valid.bool('reverse', false);
      let after = valid.brhash('after', null);
      enforce(addresses, 'Addresses are required.');
      enforce(!this.chain.options.spv, 'Cannot get TX in SPV mode.');
      for(const address of addresses) {
        let endReached = false;
        let txCounter = 0;
        while(!endReached && txCounter < MAX_TX_COUNT) {
          let addr = Address.fromString(address, this.network);
          let metas = await this.node.getMetaByAddress(
            addr, {limit, reverse, after});
          for (const meta of metas) {
            const view = await this.node.getMetaView(meta);
            const tx = meta.getJSON(this.network, view, this.chain.height);
            result.push(this.filterTxBulkResponse(tx));
            txCounter++;
          }
          if(metas.length > 0) {
            // Taking the last tx hash to know where to start from next
            let hash = metas[metas.length - 1].tx.hash();
            after = Buffer.from(hash, 'hex');
          }else{
            endReached = true;
